...
from app.core.log import logger, node_log
...
async def test_missing_run_id_is_created_and_error_log_omits_exception_text():
    ...
    try:
        result = await AgentRuntime().initialize({"trip_id": None})
    ...
    assert any("event=node_failed" in line and "error_type=RuntimeError" in line for line in lines)
...
async def test_amap_network_error_log_omits_api_key():
    ...
    lines = [str(message) for message in messages]
    assert any("event=amap_completed" in line and "status=failed" in line for line in lines)
    assert all("secret-key" not in line for line in lines)
